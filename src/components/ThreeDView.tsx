...
// Kamdo component and preload logic
...
function Kamdo(props: KamdoProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (stripe.current) stripe.current.color.setRGB(2 + t * 20, 2, 20 + t * 50);
    if (head.current) easing.dampE(head.current.rotation, [0, state.pointer.x * (state.camera.position.z > 1 ? 1 : -1), 0], 0.4, delta);
    if (light.current) light.current.intensity = 1 + t * 4;
  });
  return (
    <group {...props}>
